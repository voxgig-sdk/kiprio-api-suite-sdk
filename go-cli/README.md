# kiprio-api-suite-cli

boru-driven command-line client **and** interactive REPL for the KiprioApiSuite
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kiprio-api-suite-cli)
make build

# 2. See usage (words, entities, env vars)
./kiprio-api-suite-cli --help

# 3. Provide credentials once, via the environment
export KIPRIO_API_SUITE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./kiprio-api-suite-cli load 1 dns_result            # {id:1} shorthand
./kiprio-api-suite-cli load '{id:1}' dns_result       # explicit match map

# 5. Override the API base URL for a single call
KIPRIO_API_SUITE_BASE=https://api.example.com ./kiprio-api-suite-cli load 1 dns_result

# 6. No arguments -> interactive REPL
./kiprio-api-suite-cli
kiprio-api-suite> load 1 dns_result
kiprio-api-suite> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kiprio-api-suite-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/kiprio-api-suite-cli load 1 dns_result
   ```
...
```sh
./kiprio-api-suite-cli load 1 dns_result          # scalar shorthand for {id:1}
./kiprio-api-suite-cli load '{id:1}' dns_result     # explicit match map
```
...
./kiprio-api-suite-cli load 1 dns_result
```
...
Run with no arguments to open a REPL (prompt `kiprio-api-suite>`). Each line is
evaluated as its own boru expression:

```text
$ ./kiprio-api-suite-cli
kiprio-api-suite> load 1 dns_result
kiprio-api-suite> /help
kiprio-api-suite> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/kiprio-api-suite-cli`. |
...
(`./kiprio-api-suite-cli <expr>`) and interactively (the REPL), and expressions compose the

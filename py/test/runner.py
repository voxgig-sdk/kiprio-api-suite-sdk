# KiprioApiSuite SDK test runner
...
class KiprioApiSuiteTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            KiprioApiSuiteTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = KiprioApiSuiteTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = KiprioApiSuiteTestRunner.getenv("KIPRIO_API_SUITE_TEST_LIVE")
        override = KiprioApiSuiteTestRunner.getenv("KIPRIO_API_SUITE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = KiprioApiSuiteTestRunner.getenv(key)
        ...
        explain = KiprioApiSuiteTestRunner.getenv("KIPRIO_API_SUITE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["KIPRIO_API_SUITE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if KiprioApiSuiteTestRunner._test_control is not None:
            return KiprioApiSuiteTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                KiprioApiSuiteTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            KiprioApiSuiteTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = KiprioApiSuiteTestRunner.load_test_control()
...
def load_env_local():
    KiprioApiSuiteTestRunner.load_env_local()


def env_override(m):
    return KiprioApiSuiteTestRunner.env_override(m)


def entity_data(v):
    return KiprioApiSuiteTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return KiprioApiSuiteTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return KiprioApiSuiteTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return KiprioApiSuiteTestRunner.load_test_control()


def live_client_options():
    return KiprioApiSuiteTestRunner.live_client_options()


def live_delay_ms():
    return KiprioApiSuiteTestRunner.live_delay_ms()

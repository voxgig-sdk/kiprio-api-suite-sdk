...
//    only - a typed KiprioApiSuiteContext there would make every ctx
...
final class KiprioApiSuiteOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new KiprioApiSuiteContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new KiprioApiSuiteSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new KiprioApiSuiteResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new KiprioApiSuiteError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new KiprioApiSuiteResponse($clean['response']);
    ...
    public static function err_from_map($m): ?KiprioApiSuiteError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new KiprioApiSuiteError(is_string($code) ? $code : '', $msg);
    }

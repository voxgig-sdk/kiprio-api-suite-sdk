...
class KiprioApiSuiteSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = KiprioApiSuiteHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = KiprioApiSuiteHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!KiprioApiSuiteFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, KiprioApiSuiteFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return KiprioApiSuiteUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = KiprioApiSuiteHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = KiprioApiSuiteHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = KiprioApiSuiteHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new KiprioApiSuiteError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = KiprioApiSuiteHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new KiprioApiSuiteError("graphql_error",
                "KiprioApiSuiteSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new KiprioApiSuiteSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}

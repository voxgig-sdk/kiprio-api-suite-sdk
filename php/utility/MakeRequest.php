...
class KiprioApiSuiteMakeRequest
{
    public static function call(KiprioApiSuiteContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new KiprioApiSuiteResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new KiprioApiSuiteResponse($fetched);
        } else {

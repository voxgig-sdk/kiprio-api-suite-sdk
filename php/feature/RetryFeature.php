...
// KiprioApiSuite SDK retry feature
...
class KiprioApiSuiteRetryFeature extends KiprioApiSuiteBaseFeature
{
    ...
    public function init(KiprioApiSuiteContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (KiprioApiSuiteContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(KiprioApiSuiteContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(KiprioApiSuiteContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

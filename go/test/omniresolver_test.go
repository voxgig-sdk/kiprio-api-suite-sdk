...
import (
	...
	omni "github.com/voxgig-sdk/kiprio-api-suite-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.KiprioApiSuiteError); is {
				out := map[string]any{
					"name":    "KiprioApiSuiteError",
...
func omniCtx(arg any, client *sdk.KiprioApiSuiteSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.KiprioApiSuiteSDK); is {
			client = live

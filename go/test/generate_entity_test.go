...
func TestGenerateEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set KIPRIOAPISUITE_TEST_GENERATE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func generateBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("KIPRIOAPISUITE_TEST_GENERATE_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"KIPRIOAPISUITE_TEST_GENERATE_ENTID": idmap,
		"KIPRIOAPISUITE_TEST_LIVE":      "FALSE",
		"KIPRIOAPISUITE_TEST_EXPLAIN":   "FALSE",
		"KIPRIOAPISUITE_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["KIPRIOAPISUITE_TEST_GENERATE_ENTID"])

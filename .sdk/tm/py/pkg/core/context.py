...
from projectname_sdk.core.control import KiprioApiSuiteControl
...
class KiprioApiSuiteContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = KiprioApiSuiteControl()
        ...
        if isinstance(sp, KiprioApiSuiteSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, KiprioApiSuiteResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, KiprioApiSuiteResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return KiprioApiSuiteOperation({})
    ...
    def make_error(self, code, msg):
        return KiprioApiSuiteError(code, msg, self)

...
// KiprioApiSuite SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class KiprioApiSuiteMakePoint
{
    public static function call(KiprioApiSuiteContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof KiprioApiSuiteError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = KiprioApiSuiteHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = KiprioApiSuiteHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));

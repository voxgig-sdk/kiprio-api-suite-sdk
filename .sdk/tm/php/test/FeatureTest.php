...
// KiprioApiSuite SDK feature test
//
...
class FtCtrl extends KiprioApiSuiteControl
{
...
class FtClient
{
    ...
    private KiprioApiSuiteUtility $utility;

    public function __construct(KiprioApiSuiteUtility $utility, array $options)
    {
    ...
    public function get_utility(): KiprioApiSuiteUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (KiprioApiSuiteContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public KiprioApiSuiteContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (KiprioApiSuiteContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = KiprioApiSuiteConfig::make_feature($name);
    ...
    private static function buildUrl(KiprioApiSuiteSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof KiprioApiSuiteError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new KiprioApiSuiteResponse($fetched);
        }
    ...
    private function fail(KiprioApiSuiteContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (KiprioApiSuiteContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

...
// KiprioApiSuite SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class KiprioApiSuiteTestFeature extends KiprioApiSuiteBaseFeature
{
    ...
    public function init(KiprioApiSuiteContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (KiprioApiSuiteContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = KiprioApiSuiteParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (KiprioApiSuiteContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(KiprioApiSuiteContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = KiprioApiSuiteParam::call($ctx, $k);

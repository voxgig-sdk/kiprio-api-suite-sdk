...
(0, node_test_1.describe)('EmailValidateEntity', async () => {
    ...
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('KIPRIO_API_SUITE_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.KiprioApiSuiteSDK.test();
        const ent = testsdk.EmailValidate();
        (0, node_assert_1.default)(null != ent);
    });
    (0, node_test_1.test)('basic', async (t) => {
        const live = 'TRUE' === process.env.KIPRIO_API_SUITE_TEST_LIVE;
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set KIPRIO_API_SUITE_TEST_EMAIL_VALIDATE_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    let client = __1.KiprioApiSuiteSDK.test(options, extra);
    ...
    const idmapEnvVal = process.env['KIPRIO_API_SUITE_TEST_EMAIL_VALIDATE_ENTID'];
    const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');
    const env = (0, utility_1.envOverride)({
        'KIPRIO_API_SUITE_TEST_EMAIL_VALIDATE_ENTID': idmap,
        'KIPRIO_API_SUITE_TEST_LIVE': 'FALSE',
        'KIPRIO_API_SUITE_TEST_EXPLAIN': 'FALSE',
        'KIPRIO_API_SUITE_APIKEY': '',
    });
    idmap = env['KIPRIO_API_SUITE_TEST_EMAIL_VALIDATE_ENTID'];
    const live = 'TRUE' === env.KIPRIO_API_SUITE_TEST_LIVE;
    if (live) {
        client = new __1.KiprioApiSuiteSDK(merge([
    ...
    const setup = {
        ...
        struct,
        data: entityData,
        explain: 'TRUE' === env.KIPRIO_API_SUITE_TEST_EXPLAIN,
        live,

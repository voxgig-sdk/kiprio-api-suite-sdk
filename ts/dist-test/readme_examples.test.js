...
const __1 = require("..");
const SDK_NAME = 'KiprioApiSuiteSDK';
...
function isRunnable(code) {
    return (/new\s+KiprioApiSuiteSDK\b/.test(code) ||
        /\bKiprioApiSuiteSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+KiprioApiSuiteSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/KiprioApiSuiteSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);

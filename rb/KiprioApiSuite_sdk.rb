...
require_relative 'KiprioApiSuite_types'
...
class KiprioApiSuiteSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = KiprioApiSuiteHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = KiprioApiSuiteHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, KiprioApiSuiteFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    KiprioApiSuiteUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = KiprioApiSuiteHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = KiprioApiSuiteHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = KiprioApiSuiteHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue KiprioApiSuiteError => err
    ...
    if fetched.is_a?(Hash)
      status = KiprioApiSuiteHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = KiprioApiSuiteError.new(
        "graphql_error", "KiprioApiSuiteSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = KiprioApiSuiteSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end

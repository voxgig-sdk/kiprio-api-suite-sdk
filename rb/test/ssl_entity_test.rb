...
class SslEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set KIPRIOAPISUITE_TEST_SSL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def ssl_basic_setup(extra)
  ...
  entid_env_raw = ENV["KIPRIOAPISUITE_TEST_SSL_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "KIPRIOAPISUITE_TEST_SSL_ENTID" => idmap,
  ...
  idmap_resolved = Helpers.to_map(
    env["KIPRIOAPISUITE_TEST_SSL_ENTID"])

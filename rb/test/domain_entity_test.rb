...
class DomainEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = KiprioApiSuiteSDK.test(seed, nil)
    seen = base.Domain(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = KiprioApiSuiteConfig.make_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = KiprioApiSuiteSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set KIPRIOAPISUITE_TEST_DOMAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def domain_basic_setup(extra)
  ...
  client = KiprioApiSuiteSDK.test(options, extra)
  ...
  entid_env_raw = ENV["KIPRIOAPISUITE_TEST_DOMAIN_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "KIPRIOAPISUITE_TEST_DOMAIN_ENTID" => idmap,
  ...
  idmap_resolved = Helpers.to_map(
    env["KIPRIOAPISUITE_TEST_DOMAIN_ENTID"])
  ...
  if env["KIPRIOAPISUITE_TEST_LIVE"] == "TRUE"
    ...
    client = KiprioApiSuiteSDK.new(Helpers.to_map(merged_opts))
  end

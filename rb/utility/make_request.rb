...
module KiprioApiSuiteUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = KiprioApiSuiteResponse.new({})
    ...
    if fetch_err
      ...
      response = KiprioApiSuiteResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = KiprioApiSuiteResponse.new(fetched)
    else

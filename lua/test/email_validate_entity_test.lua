...
describe("EmailValidateEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set KIPRIO_API_SUITE_TEST_EMAIL_VALIDATE_ENTID JSON to run live")
      return
    end
...
function email_validate_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("KIPRIO_API_SUITE_TEST_EMAIL_VALIDATE_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["KIPRIO_API_SUITE_TEST_EMAIL_VALIDATE_ENTID"])
  if idmap_resolved == nil then
    idmap_resolved = helpers.to_map(idmap)
  end

  if env["KIPRIO_API_SUITE_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["KIPRIO_API_SUITE_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,

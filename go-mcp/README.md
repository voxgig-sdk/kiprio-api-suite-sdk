# kiprio-api-suite-mcp

[MCP](https://modelcontextprotocol.io) server exposing the KiprioApiSuite SDK as
two agent tools — `kiprio-api-suite_list` and `kiprio-api-suite_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/kiprio-api-suite-mcp)
make build

# 2. Provide credentials via the environment
export KIPRIO_API_SUITE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user kiprio-api-suite \
  -- /absolute/path/to/kiprio-api-suite-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./kiprio-api-suite-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// kiprio-api-suite_list: first page of records
{ "entity": "domain" }
{ "entity": "domain", "query": { } }

// kiprio-api-suite_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/kiprio-api-suite-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user kiprio-api-suite \
     -- "$PWD"/dist/*/kiprio-api-suite-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `kiprio-api-suite_list` and `kiprio-api-suite_load` tools now appear
   in new sessions. Ask the agent to *"list domain using kiprio-api-suite"*
   and it calls `kiprio-api-suite_list` with `{"entity":"domain"}`.
...
```sh
export KIPRIO_API_SUITE_APIKEY=sk_live_xxx            # API key
export KIPRIO_API_SUITE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `kiprio-api-suite_list` tool
...
### Call the `kiprio-api-suite_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `kiprio-api-suite_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `kiprio-api-suite_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `KIPRIO_API_SUITE_APIKEY` | API key sent with every request. |
| `KIPRIO_API_SUITE_BASE` | Optional override of the API base URL. |
...
```sh
./kiprio-api-suite-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"kiprio-api-suite_load","arguments":{"entity":"dns_result","query":{"id":1}}}}'
```
